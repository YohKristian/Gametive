						...
						{yourEvents.items.map((event, idx) => (
							<div className="search-item" key={idx}>
								...
								<div>
									...
									<div>
										<p>
											<i className="bi bi-geo-alt-fill"></i> {event.Location.name}
										</p>
										<p>
											<i className="bi bi-flag-fill"></i> {dateFormat(event.eventDate)}
										</p>
										<p>
											<i className="bi bi-cash"></i> {event.price === 0 ? "Free" : rupiahFormat(event.price)}
										</p>
										<p>
							...
							<ul className="pagination">
								...
								<li className="page-item pagination">
									{pageNumber(yourEvents.totalPages).map((x, idx) => {
										return (
											<a key={idx} className="page-link" onClick={() => handlePage(x)}>
												{x}
											</a>
										);
									})}
